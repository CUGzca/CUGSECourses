...
def val_extract_feature(start,end,file_name):
    ...
    for j in find_list[start:end+1]:
        ...
        if len(batch)==8:
            ...
            batch.clear()
            out=model(torch.from_numpy(input_).float()).detach()
...
def get_caption_a_video(ambit,file_name):
    ans=''
    start,end=ambit
    features=val_extract_feature(start,end,file_name)
    features=torch.FloatTensor(features)
    s2vt = S2VT(vocab_size=vovab_size, batch_size=1)
    #s2vt = s2vt.cuda()
    s2vt.load_state_dict(torch.load("/root/s2vt_params.pkl", map_location='cpu'))
    s2vt.eval()
    for param in s2vt.parameters():
        param.require_grad=False
